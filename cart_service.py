...
cart_service = Flask(__name__)

# Sample user cart data (simulated for testing)
user_carts = {
    1: {1: 3, 2: 1},  # User 1 has 3 cart_serviceles and 1 Bananas in their cart
...
@cart_service.route('/cart/<int:user_id>', methods=['GET'])
...
@cart_service.route('/cart/<int:user_id>/add/<int:product_id>', methods=['POST'])
...
@cart_service.route('/cart/<int:user_id>/remove/<int:product_id>', methods=['POST'])
    ...
    if 'quantity' in data:
        ...
        if product_response.status_code == 200:
            ...
            if user_id in user_carts and product_id in user_carts[user_id]:
                current_quantity = user_carts[user_id][product_id]
                if current_quantity >= quantity:
                    user_carts[user_id][product_id] -= quantity
                    if user_carts[user_id][product_id] <= 0:
                        del user_carts[user_id][product_id]
                    return jsonify({"message": f"Removed {quantity} {product_name} from the cart."}), 200
                else:
                    return jsonify({"message": "Insufficient quantity in the cart"}), 400
            else:
...
if __name__ == '__main__':
    cart_service.run(host='0.0.0.0', port=5001)